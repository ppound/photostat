...
Download `PhotoStat-2.6.3.msi`, double-click to install, and launch from the Start Menu. No Java installation required.

#### Option B: macOS Installer (.dmg) — Apple Silicon only

Download `PhotoStat-2.6.3-apple-silicon.dmg`, open it, and drag PhotoStat to your Applications folder. No Java installation required.
...
>
> **Intel Mac users:** A DMG installer is not available for Intel Macs. Use the cross-platform JAR below (`photostat-java-2.6.3-executable-mac-intel.jar`).

#### Option C: Cross-platform JAR

Download `photostat-java-2.6.3-executable.jar`. Requires Java 21+. This JAR includes native libraries for **Windows**, **Linux**, and **macOS Apple Silicon** (M1/M2/M3/M4).

```bash
java -jar photostat-java-2.6.3-executable.jar
```

**Intel Mac users:** Download the separate `photostat-java-2.6.3-executable-mac-intel.jar` which includes Intel (x86_64) macOS natives instead of Apple Silicon. See [Troubleshooting](docs/TROUBLESHOOTING.md#error-on-intel-mac-no-suitable-pipeline-found-or-graphics-errors).
...
java -jar photostat-java-2.6.3-executable.jar --analyze

# Preview what would be analyzed
java -jar photostat-java-2.6.3-executable.jar --analyze --dry-run

# Run with 4 parallel threads
java -jar photostat-java-2.6.3-executable.jar --analyze --parallel 4

# Use Gemini instead of Claude
java -jar photostat-java-2.6.3-executable.jar --analyze --provider gemini

# Use Ollama (local)
java -jar photostat-java-2.6.3-executable.jar --analyze --provider ollama

# Find duplicate images
java -jar photostat-java-2.6.3-executable.jar --find-duplicates

# Find visually similar images
java -jar photostat-java-2.6.3-executable.jar --find-duplicates --mode visual
...
java -jar photostat-java-2.6.3-executable.jar --detect-faces --dir /path/to/photos

# Score photos for aesthetic quality (requires the aesthetic Docker backend)
java -jar photostat-java-2.6.3-executable.jar --score-aesthetics

# Re-score everything, larger batches
java -jar photostat-java-2.6.3-executable.jar --score-aesthetics --force --batch 32

# Upload to cloud via rclone
java -jar photostat-java-2.6.3-executable.jar --rclone-upload

# Preview what would be uploaded
java -jar photostat-java-2.6.3-executable.jar --rclone-upload --dry-run
```
